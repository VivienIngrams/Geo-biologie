  ...
  return (
    <nav className="absolute md:w-[80%] md:ml-[10%] z-50 text-black font-medium tracking-tight">
      ...
      >
        <div className="flex items-center md:-ml-12">
          <div className="hidden md:block">
            <div className="flex items-baseline">Links</div>
          </div>
        </div>
      ...
      <div
        className={`${
          isOpen
            ? 'block bg-gradient-to-b from-transparent via-white to-white w-screen pt-24 -ml-4'
            : 'hidden'
        } md:hidden`}
        id="mobile-menu"
        onClick={toggleMenu}
      >
        <div className="text-black px-2 pt-2 pb-3 space-y-1 text-md sm:px-3">Links</div>
      </div>
    </nav>
  )
}
